...
async function exportConversations(accessToken, progress) {
  ...
  for (let i = 0; i < conversations.length; i++) {
    ...
    // 13 chars spans first UUID segment + dash + second segment start; prevents silent-skip collisions on timestamp-encoded IDs.
...
// Scans JSON files on disk (regular + project dirs) and returns the set of conversation IDs present.
...
// Removes IDs from progress.downloadedIds (and per-project arrays) that have no file on disk,
// so the main export loop re-fetches them.
