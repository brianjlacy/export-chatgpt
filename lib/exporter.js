...
// of conversation IDs actually present. Used by refetch-missing to detect
// any progress entries that claim "downloaded" but have no file.
...
// Removes any IDs from progress.downloadedIds that don't have a file on disk
// so the main export loop re-fetches them. Mutates progress; saves to disk.
function refetchMissing(progress) {
  const onDisk = collectIdsOnDisk();
  const toRefetch = (progress.downloadedIds || []).filter(id => !onDisk.has(id));
  if (toRefetch.length === 0) {
    console.log('  No missing conversations — progress and disk are in sync.\n');
    return 0;
  }
  const toRefetchSet = new Set(toRefetch);
  progress.downloadedIds = progress.downloadedIds.filter(id => !toRefetchSet.has(id));
  // Also clear from per-project downloadedIds so project-scoped missing convos get retried.
  if (progress.projects) {
    for (const pid of Object.keys(progress.projects)) {
      ...
      if (pp && Array.isArray(pp.downloadedIds)) {
        pp.downloadedIds = pp.downloadedIds.filter(id => !toRefetchSet.has(id));
      }
    }
  }
  saveProgress(progress);
  console.log(`  Marked ${toRefetch.length} silently-skipped conversation(s) for re-download.\n`);
  return toRefetch.length;
}
...
function printSummary(summary) {
  ...
  // Files line (only if file downloads were enabled and any were downloaded or retried)
  if (CONFIG.downloadFiles && (files > 0 || summary.retriedFiles > 0)) {
