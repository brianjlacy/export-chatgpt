...
// Scans progress + disk and returns IDs marked downloaded but with no file.
// Used by --verify to report silent skips caused by filename collisions or
// mid-run write failures. Scans both regular and project JSON directories.
function findSilentlySkippedConversations(progress) {
  const onDisk = new Set();
  if (fs.existsSync(PATHS.jsonDir)) {
    for (const f of fs.readdirSync(PATHS.jsonDir)) {
      if (!f.endsWith('.json')) continue;
      try {
        const data = JSON.parse(fs.readFileSync(path.join(PATHS.jsonDir, f), 'utf8'));
  ...
  if (fs.existsSync(PATHS.projectsDir)) {
    for (const projFolder of fs.readdirSync(PATHS.projectsDir)) {
      const projJsonDir = path.join(PATHS.projectsDir, projFolder, 'json');
      if (!fs.existsSync(projJsonDir)) continue;
      for (const f of fs.readdirSync(projJsonDir)) {
        if (!f.endsWith('.json')) continue;
        try {
          const data = JSON.parse(fs.readFileSync(path.join(projJsonDir, f), 'utf8'));
          const id = data.id || data.conversation_id;
          if (id) onDisk.add(id);
        } catch {}
      }
...
module.exports = { exportConversations, exportProjectConversations, run, printSummary, findSilentlySkippedConversations };
