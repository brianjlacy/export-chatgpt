...
// "You don't have access to this conversation" is the server stating a stable
// fact about that conversation, not an ambiguous edge response — it reproduces
// from the browser. Still requires two separate runs before skipping, so a
// one-off never writes a conversation off.
const MIN_NO_ACCESS_RUNS_BEFORE_SKIP = 2;
...
function recordConversationFailure(progress, conv, error, tracker) {
  ...
  // A run of legitimate access denials is not a session-level collapse.
  if (error.accessDenied && tracker) tracker.streak = [];
  ...
  const entry = {
    ...
    reason: error.accessDenied ? 'no_access' : 'not_found',
    title: conv.title || null,
    attempts: (prior && prior.attempts ? prior.attempts : 0) + 1,
    firstFailedAt: (prior && prior.firstFailedAt) || now,
    lastFailedAt: now,
  };
  // Carry the collapse tag forward, but a definitive no_access supersedes it:
  // the server has now named a stable, conversation-specific reason, so the
  // earlier ambiguous streak tells us nothing more. Without this, anything
  // recorded during a collapse and later classified stays pinned as
  // "always retry" forever — the exact waste this is meant to prevent.
  if (prior && prior.duringRunCollapse && !error.accessDenied) {
    entry.duringRunCollapse = true;
  }
  progress.failedConversationIds[conv.id] = entry;

  if (tracker && !error.accessDenied) {
...
// Describe what will actually happen to this ID next run, so the log never
// claims something the skip rules don't do.
function describeFailureHandling(progress, id) {
  const entry = (progress.failedConversationIds || {})[id];
  if (!entry) return 'Recorded.';
  const attempts = entry.attempts || 0;

  if (entry.duringRunCollapse) {
    return `Recorded (part of a run-level failure streak) — always retried, never skipped.`;
  }
  if (entry.reason === 'no_access') {
    const left = MIN_NO_ACCESS_RUNS_BEFORE_SKIP - attempts;
    return left > 0
      ? `Recorded as no_access (attempt ${attempts}) — retried ${left} more run(s), then skipped.`
      : `Recorded as no_access (attempt ${attempts}) — skipped on future runs (--retry-all to force).`;
  }
  if (!CONFIG.skipFailedConversations) {
    return `Recorded as not_found (attempt ${attempts}) — retried next run. A 404 is not proof of deletion.`;
  }
  const left = MIN_FAILED_RUNS_BEFORE_SKIP - attempts;
  return left > 0
    ? `Recorded as not_found (attempt ${attempts}) — retried ${left} more run(s), then skipped.`
    : `Recorded as not_found (attempt ${attempts}) — skipped on future runs (--skip-failed is set).`;
}
...
// Two tiers, because the two kinds of 404 carry very different confidence:
//
//   no_access  — the server named the reason and it reproduces everywhere.
//                Skipped by default after 2 runs; --retry-all forces retry.
//   not_found  — ambiguous. Only skipped if the user opts in with
//                --skip-failed, only after 3 runs, and never when it was part
//                of a run-level collapse.
function isKnownDeadConversation(progress, id) {
  ...
  if (entry.reason === 'no_access') {
    if (CONFIG.retryAllFailed) return false;
    return (entry.attempts || 0) >= MIN_NO_ACCESS_RUNS_BEFORE_SKIP;
  }
  ...
  for (let i = 0; i < conversations.length; i++) {
    ...
    try {
      ...
      if (recordConversationFailure(progress, conv, error, failureTracker)) {
        verboseLog(`    ${describeFailureHandling(progress, conv.id)}`);
      }
      errorCount++;
    }
  }
...
async function runVerify(progress) {
  ...
  if (dead.length > 0) {
    const na = dead.filter(([, i]) => i.reason === 'no_access').length;
    console.log(`Returned HTTP 404:       ${dead.length} total`);
    console.log(`  no access (stable):    ${na} — this account cannot read these`);
    console.log(`  not found (ambiguous): ${dead.length - na} — retried automatically`);
...
async function run(accessToken) {
  ...
  try {
    ...
    const deadEntries = Object.values(progress.failedConversationIds || {});
    if (deadEntries.length > 0) {
      summary.failedConversations = deadEntries.length;
      summary.failedNoAccess = deadEntries.filter(e => e.reason === 'no_access').length;
    }
  } catch (error) {
    if (error.authError) {
...
function printSummary(summary) {
  ...
  if (deadTotal > 0) {
    const noAccess = summary.failedNoAccess || 0;
    const notFound = deadTotal - noAccess;
    console.log(`  404 responses:  ${deadTotal} total — see .export-progress.json → failedConversationIds`);
    if (noAccess > 0) {
      console.log(`    no access:    ${noAccess} — server says this account cannot read them.`);
      console.log(`                  Stable; skipped on later runs (--retry-all to override).`);
    }
    if (notFound > 0) {
      console.log(`    not found:    ${notFound} — ambiguous. Retried automatically next run.`);
      console.log(`                  Check one at https://chatgpt.com/c/<id> before assuming loss.`);
    }
  }
