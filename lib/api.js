...
// Returns a numeric Unix timestamp (seconds) regardless of whether
// the API returned a number (e.g. 1700000000) or an ISO string
// (e.g. "2023-11-14T22:13:20.000Z"). Returns 0 on any parse failure
// so that unknown values are treated as "already indexed".
...
async function fetchConversationListIncremental(accessToken, existingIndex, progress) {
  ...
  if (progress.indexingComplete) {
    return fetchNewConversations(accessToken, existingIndex, progress);
  }
...
async function fetchNewConversations(accessToken, existingIndex, progress) {
  let since = 0;
  for (const conv of existingIndex.values()) {
    const t = toUnixSeconds(conv.create_time);
    if (t > since) since = t;
  }
  ...
  while (true) {
    const url = `${CONFIG.apiBase}/conversations?offset=${offset}&limit=${CONFIG.conversationsPerPage}&order=created`;
    ...
    try {
      ...
      if (hitWatermark || data.items.length < CONFIG.conversationsPerPage) break;

      offset += data.items.length;
    } catch (error) {
      if (error.authError) {
        console.log('\n  Token expired during delta check. Progress saved.');
        throw error;
      }
      throw error;
    }
  ...
  if (newCount > 0) {
    saveIndex(existingIndex);
    saveProgress(progress);
    console.log(`  Found ${newCount} new conversation${newCount === 1 ? '' : 's'}\n`);
  } else {
