...
// File IDs are not URL-safe. ChatGPT composes some of them from several parts
// joined by '#' (e.g. "1db2089...#file_00000000845...#p_0.ecfab8dfdb.jpg").
// Interpolated raw, the first '#' starts the URL fragment: everything after it
// — including the ?conversation_id= query string — is dropped by the client and
// never sent. The server then sees a request with no conversation_id and
// answers 422 Unprocessable Entity, so every such file fails permanently.
async function getFileDownloadUrl(accessToken, fileId, conversationId) {
  const url = `${CONFIG.apiBase}/files/download/${encodeURIComponent(fileId)}`
...
async function retryPendingFiles(accessToken, progress) {
  ...
  for (const conv of mainIndex.values()) {
    ...
    for (const ref of conv.files) {
      if (!progress.downloadedFileIds.includes(ref.fileId) && !progress.failedFileIds[ref.fileId] && passesFilter(ref.type)) {
