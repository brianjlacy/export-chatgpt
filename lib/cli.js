...
function setupCLI() {
  ...
  program
    ...
    .option('--throttle <ms>', 'Minimum time between requests in ms — ChatGPT rate-limits excessive requests, so a higher value reduces errors (default: 60000)', '60000')
...
async function main() {
  ...
  CONFIG.throttleMs = parseInt(opts.throttle, 10);
  if (isNaN(CONFIG.throttleMs) || CONFIG.throttleMs < 0) {
    console.warn(`Warning: Invalid --throttle value "${opts.throttle}", using default 60000ms`);
