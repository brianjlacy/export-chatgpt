...
function setupCLI() {
  ...
  program
    ...
    .option('--skip-failed', 'Skip conversations that returned HTTP 404 on 3 or more separate runs. Off by default: a 404 can come from edge/bot blocking rather than a missing conversation, so failures are retried unless you opt out.', false)
    .option('--retry-all', 'Also re-attempt conversations the server reported as inaccessible to this account. Those are stable failures and are skipped after 2 runs by default.', false)
...
async function main() {
  ...
  CONFIG.skipFailedConversations = !!opts.skipFailed;
  CONFIG.retryAllFailed = !!opts.retryAll;
