...
function setupCLI() {
  ...
  program
    ...
    .option('--include-archived', 'Also fetch archived conversations (excluded by default — the API omits them unless explicitly requested)', false)
    .option('--verify', 'Dry-run: report conversations marked downloaded but missing from disk, then exit. No network calls, no modifications.', false)
    .option('--refetch-missing', 'Before running, clear from progress any downloaded IDs with no file on disk so the export loop re-fetches them', false)
