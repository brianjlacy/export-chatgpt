...
async function fetchWithRetry(url, options, retries = 6) {
  for (let i = 0; i < retries; i++) {
    try {
      ...
      if (elapsed < CONFIG.throttleMs) await sleep(CONFIG.throttleMs - elapsed);
...
module.exports = { createApiHeaders, fetchWithRetry, extractAccountIdFromJWT, getAccessToken };
