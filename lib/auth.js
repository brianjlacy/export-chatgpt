...
function createApiHeaders(accessToken) {
  // Cloudflare bot-fingerprinting at chatgpt.com/backend-api rejects requests
  // that don't look browser-shaped. Sending Bearer + Account-Id alone returns
  // 401 from Node even when the same token returns 200 from the browser.
  // Mirroring the browser's User-Agent + Referer + Origin gets us through
  // the edge fingerprint check.
  const headers = {
