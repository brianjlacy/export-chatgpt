  ...
  for (let i = 0; i < retries; i++) {
    try {
      ...
      if (response.status === 404) {
        // The body distinguishes two very different situations that share a
        // status code:
        //   "You don't have access to this conversation" — stable and
        //   conversation-specific; fails identically from the browser.
        //   A bare 404 — usually the client's fault (edge/bot rejection,
        //   which climbs the longer a run goes) and retryable.
        // Without reading the body they are indistinguishable, and treating
        // the second kind as permanent causes silent data loss.
        const body = await response.text().catch(() => '');
        const accessDenied = /don.t have access to this conversation/i.test(body);
        verboseLog(`    HTTP 404 ${accessDenied ? '(no access)' : '(not found)'} — ${url}`);
        const error = new Error(accessDenied
          ? 'HTTP 404: No access to this conversation'
          : 'HTTP 404: Not Found');
        error.noRetry = true;
        error.notFound = true;
...
module.exports = { createApiHeaders, fetchWithRetry, throttle, extractUserIdFromJWT, extractAccountIdFromJWT, getAccessToken, verifyToken, noteRateLimit, noteSuccess };
